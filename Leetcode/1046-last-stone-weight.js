...
var lastStoneWeight = function (stones) {
  while (stones.length > 1) {
    // find two heaviest stones - O(N)
    let heaviest = 0;
    let heaviestIdx = 0;
    let secondHeaviest = 0;
    let secondHeaviestIdx = 0;

    for (let i = 0; i < stones.length; i++) {
      if (stones[i] >= heaviest) {
        secondHeaviest = heaviest;
        secondHeaviestIdx = heaviestIdx;
        heaviest = stones[i];
        heaviestIdx = i;
      } else if (stones[i] >= secondHeaviest) {
        secondHeaviest = stones[i];
        secondHeaviestIdx = i;
      }
    }

    // remove the two heaviest stones from array - O(N)
    if (heaviestIdx > secondHeaviestIdx) {
      stones.splice(heaviestIdx, 1);
      stones.splice(secondHeaviestIdx, 1);
    } else {
      stones.splice(secondHeaviestIdx, 1);
      stones.splice(heaviestIdx, 1);
    }

    // smash and push back to array - O(N)
    let result = Math.abs(heaviest - secondHeaviest);
    if (result === 0 && stones.length === 0) return 0;
    else if (result > 0) stones.push(result);
  }
